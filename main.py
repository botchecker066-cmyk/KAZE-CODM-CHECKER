...
def init_db():
    ...
    if conn:
        ...
        columns = [info['name'] for info in cursor.fetchall()]
        if 'checker_type' not in columns:
...
def generate_dashboard(processed, total, valid, invalid, current_account, current_status):
    ...
    panel = Panel(
        table, 
        title="[bold white]✧ KAZE CODM CHECKER Status ✧[/bold white]", 
        border_style="cyan",
        box=ROUNDED
    )
    return Group(panel, "")
...
class LiveStats:
    ...
    def update_stats(self, valid=False, clean=False, has_codm=False, level=0, server='N/A'):
        with self.lock:
            if valid:
                self.stats['valid'] += 1
                if clean: self.stats['clean'] += 1
                else: self.stats['not_clean'] += 1
    ...
    def get_stats(self):
        with self.lock:
            # Ibinabalik ang tatlong kailangan ng bot_process_checker
...
def create_account_panel(account_details, codm_info=None):
    ...
    panel = Panel(
        table,
        title="[bold white]✧ NEXUS CODM CHECKER ✧[/bold white]",
...
def save_account_details(account, details, codm_info=None, password=None, base_results_dir="Results"):
    try:
        ...
        # Pagkuha ng mga basic details para sa blocks at lines
        ...
        # 1. GENERATE THE ONE-LINE STRING FORMAT
        account_log = f"{account}"
        if password:
            account_log = f"{account}:{password}"
            
        c_region = str(codm_region).upper().strip()
        one_line_log = f"{account_log} | Region: {c_region} | Level: {codm_level} | Bind: {bind_details}\n"

        # 2. GENERATE THE FULL DETAILS BLOCK FORMAT
        full_info_str = "============================================================\n"
        full_info_str += f"Account: {account}\n"
        if password:
            full_info_str += f"Password: {password}\n"
        ...
        if facebook_connected or facebook_account:
            full_info_str += f"Facebook Username: {facebook_username}\n"
            full_info_str += f"Facebook UID: {facebook_uid}\n"                                    
        full_info_str += f"Phone: {mobile_no}\n"
        full_info_str += f"Country: {country}\n"
        full_info_str += f"Shell Balance: {shell_balance}\n"
        full_info_str += f"Account Status: {account_status}\n"
        full_info_str += f"Bind Status: {bind_details}\n"
        full_info_str += f"Security Status: {details.get('security_status', 'N/A')}\n"
        full_info_str += f"CODM Name: {codm_name}\n"
        full_info_str += f"CODM UID: {codm_uid}\n"
        full_info_str += f"CODM Region: {codm_region}\n"
        full_info_str += f"CODM Level: {codm_level}\n"
        full_info_str += "============================================================\n\n"

        # ---- LAGING MAPUPUNTA LAHAT NG VALID SA MAIN full_details.txt ----
        with open(os.path.join(base_results_dir, 'full_details.txt'), 'a', encoding='utf-8') as f_details:             
        ...
        if is_account_clean:
            ...
            target_sub_dir = os.path.join(base_results_dir, 'Clean') # Folder name: Clean
        else:
            root_file_path = os.path.join(base_results_dir, 'notclean.txt')
            target_sub_dir = os.path.join(base_results_dir, 'NotClean') # Folder name: NotClean

        # Isulat ang one-liner sa root text files (clean.txt o notclean.txt)
        ...
        if codm_info and c_region and c_region != 'N/A':
            # Gumawa ng folder para sa Bansa sa loob ng Clean o NotClean folder (e.g., Results/Clean/PH/)
            country_folder_path = os.path.join(target_sub_dir, c_region)
            os.makedirs(country_folder_path, exist_ok=True)

            # Pagkuha ng Level bracket para sa filename (e.g., 1-50_accounts.txt)
            try:
                lvl = int(codm_level)
            ...
            # Pangalan ng mismong account text file sa loob ng bansa (e.g., Results/Clean/PH/1-50_accounts.txt)
...
def main_menu(user_id=None):
    ...
    if user_id and is_admin(user_id):
        markup.add(InlineKeyboardButton("[ ᴀᴅᴍɪɴ ᴘᴀɴᴇʟ ]", callback_data="admin_panel"))
...
👋 Welcome! You have successfully accessed the Kaze CODM Checker system.

⚡ This bot provides fast, accurate, and secure checking for your files.

📂 To begin, simply send your .txt file and wait for the system to process it.

🔒 Please make sure your file is properly formatted for smooth and correct results.

💰 ᴄᴏɪɴ ʙᴀʟᴀɴᴄᴇ: {user.get('coins', 0)}

🚀 Thank you for using Kaze CODM Checker Bot. Enjoy your experience!""",
...
👋 Welcome back to the main menu of @KAZEHAYAMODZ CODM Checker.
...
💰 ᴄᴏɪɴ ʙᴀʟᴀɴᴄᴇ: {user.get('coins', 0)}""",
        ...
        text = f"ᴋɪɴᴅʟʏ ᴄᴏɴᴛᴀᴄᴛ @KAZEHAYAMODZ ғᴏʀ ɢᴄᴀsʜ ɪɴғᴏʀᴍᴀᴛɪᴏɴ.\n\n" \
               f"• ɴᴏᴛᴇ: ᴅᴏ ɴᴏᴛ ᴄᴏɴғɪʀᴍ ʏᴇᴛ. ᴄᴏɴᴛᴀᴄᴛ KAZE ғɪʀsᴛ, ʀᴇᴄᴇɪᴠᴇ ᴛʜᴇ ᴅᴇᴛᴀɪʟs, ᴛʜᴇɴ ᴘʀᴏᴄᴇᴇᴅ ᴡɪᴛʜ ᴘᴀʏᴍᴇɴᴛ."
...
def process_announcement(message):
    ...
    formatted_message = f"ᴏᴡɴᴇʀ ᴀɴɴᴏᴜɴᴄᴇᴍᴇɴᴛ - ( KAZE)\n\n{announcement_text}"
    ...
    try:
        while (any(t.is_alive() for t in threads) or not results_queue.empty()) and not stop_event.is_set():
            while not results_queue.empty() and not stop_event.is_set():
                ...
                # Kinukuha ang tatlong dictionaries mula sa LiveStats
                stats, lvls, srvs = live_stats.get_stats()
        ...
        # Kinukuha ang final data
        final_stats, final_lvls, final_srvs = live_stats.get_stats()
        ...
        markup = InlineKeyboardMarkup().row(InlineKeyboardButton("[ ʙᴀᴄᴋ ]", callback_data="main_menu"))
        try:
...
def create_forced_socks5_session():
    # Ang mismong paid SOCKS5 proxy mo
    raw_proxy = "socks5://ax_s13nmy1p:wf27yv37fnbr@165.245.185.214:25587"
    
    # PINAKAMAHALAGANG PRODUKTO: Pinapalitan natin ng 'socks5h://'
    # para pilitin ang DNS resolution na pumasok sa loob ng proxy at iwasan ang Render IP leak.
    forced_proxy_url = raw_proxy.replace("socks5://", "socks5h://")
    
    # Gumawa ng custom proxies dictionary
    proxies_dict = {
        "http": forced_proxy_url,
        "https": forced_proxy_url
    }
    
    # Kung cloudscraper ang gamit mo para sa Datadome/Garena:
    scraper = cloudscraper.create_scraper()
    scraper.proxies = proxies_dict
    
    # MAGDAGDAG NG TIMEOUT: Ito ang harang natin. 
    # Kung sakaling i-block ng Render firewall ang port mo, mag-e-error ito 
    # sa halip na gumamit ng totoong IP ng Render (iwas invalid-all burst).
    
    return scraper

# Halimbawa sa pagtawag habang nag-t-test ka:
# session = create_forced_socks5_session()
# response = session.get("https://api.ipify.org?format=json", timeout=15)

# ==========================================================
# END NG IYONG CODM HANDLERS / LAST FUNCTION MO
# ==========================================================
